                ...
                if (RFC.substring(2, 3) == (ape2.substring(0, 1).toUpperCase())) {
                    if (RFC.substring(3, 4) == (nam.substring(0, 1).toUpperCase())) {
                        if ((RFC.substring(4, 6) == fech.substring(2, 4)) && (RFC.substring(6, 8) == fech.substring(8)) && (RFC.substring(8, 10) == fech.substring(5, 7)))
                            ...
                            alert("Verifica la fecha ingresada")
                    }
                    else
                        alert("La primer letra de su nombre no coincide con el RFC ingresado")

                }
                else
                    aler("La primer letra de su apellido materno no coincide con el RFC ingresado")
            }
            else
                alert("Las primeras 2 letras no coinciden con el apellido ingresado")
        }
        else
            alert("Formato incorrecto. Verifíca el RFC ingresado");
    }
...
function validaFecha(valor) {
    ...
    if (valor.match(/^\d{4}\-\d{2}\-\d{2}$/)) {
        ...
        alert("Ingresa un formato de fecha válido");
    }
    return dConvertida;
}